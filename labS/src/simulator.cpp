...
void virtual_machine_tp::UpdateCondRegister(int regname) {
    ...
    // TO BE DONE
    int bit_15=(reg[regname]>>15)&0x1;
    if(bit_15){
        reg[R_COND]=reg[R_COND]&0b1111111111111000;
        reg[R_COND]=reg[R_COND]|0b0000000000000100;
    }
    else{
        int bit_rest=reg[regname]&0x32767;
        if(bit_rest>0){
            reg[R_COND]=reg[R_COND]&0b1111111111111000;
            reg[R_COND]=reg[R_COND]|0b0000000000000001;
        }
        else{
            reg[R_COND]=reg[R_COND]&0b1111111111111000;
            reg[R_COND]=reg[R_COND]|0b0000000000000010;
        }
    }
}
